...
export function NewEventModal({ open, onClose, initialDate, initialTime, onEventCreated }: NewEventModalProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      await calendarFetch('/calendar/events', {
        method: 'POST',
        body: {
          title: title.trim(),
          date,
      ...
      toast.success('Evento creado', {
        description: `"${title}" se agregó al calendario.`,
      })
  ...
  const headerContent = (
    <div className="flex items-start gap-3">
      <div className="size-10 rounded-xl bg-purple-500/20 flex items-center justify-center shrink-0 mt-0.5">
        <Sparkles className="size-5 text-purple-400" />
      </div>
      <div className="flex-1 min-w-0">
        <div className="mb-2">
          <span className="inline-block text-xs bg-primary/20 text-primary font-semibold px-2 py-0.5 rounded-full">
            NellaSales
          </span>
        </div>
  ...
  const footerContent = (
    <div className="flex justify-end gap-3">
      ...
      <Button
        ...
        disabled={!title.trim() || !client.trim() || isSubmitting}
